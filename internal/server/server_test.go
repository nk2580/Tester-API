...
func testApp(t *testing.T) *server.App {
    t.Helper()
    gin.SetMode(gin.TestMode)

    dsn := fmt.Sprintf("file:%s?mode=memory&cache=private", t.Name())
    db, err := datasqlite.Open(dsn)
    if err != nil {
    ...
    tokenSvc := auth.NewJWTService([]byte(testSecret), time.Hour, func() time.Time {
        return fixedNow
    })

    return server.NewApp(server.Dependencies{
        UserStore:    store,
        PingStore:    store,
        TokenService: tokenSvc,
        TimeProvider: func() time.Time { return fixedNow },
    })
}
...
func TestAuthMiddlewareMissingInvalidExpiredAndValidToken(t *testing.T) {
    ...
    expiredClaims := jwt.RegisteredClaims{
        Subject:   strconv.FormatUint(1, 10),
        IssuedAt:  jwt.NewNumericDate(fixedNow.Add(-2 * time.Hour)),
        ExpiresAt: jwt.NewNumericDate(fixedNow.Add(-1 * time.Hour)),
    }
    expiredToken, err := jwt.NewWithClaims(jwt.SigningMethodHS256, expiredClaims).SignedString([]byte(testSecret))
    if err != nil {
